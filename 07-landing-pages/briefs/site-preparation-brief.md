...
**Built from:** `01-research/page-research/site-preparation/` full research pack (files 00-08 + entity-inventory.csv + 01-keyword-universe.csv), cross-checked against `01-research/page-research/home/entity-inventory.csv` for canonical entity reuse, `business-info.md`, `01-research/topical-map-2026-09-03.md` §6.1, `docs/design-system.md`, and the current live page at `site/src/pages/site-preparation-contractor-eugene-oregon.astro`.
...
| **Current live URL** | `/site-preparation-contractor-eugene-oregon/` (the file lives at `site/src/pages/site-preparation-contractor-eugene-oregon.astro`) — **flag to David/dev before build:** the topical map (`01-research/topical-map-2026-09-03.md` §5, §18) specifies flat URLs (`/site-preparation/`, not `/site-preparation-contractor-eugene-oregon/`). This brief targets the flat URL per the task instruction and the topical map; resolving the URL structure (move the file, or update the topical map to match the nested structure, plus a 301 if the nested URL is ever indexed) is a pre-publish decision this brief does not make unilaterally. |
...
- **`BreadcrumbList`** — Home → Services → Site Preparation, matching the current live page's already-implemented `Breadcrumbs` component (note: if the URL moves from `/site-preparation-contractor-eugene-oregon/` to `/site-preparation/` per §1's flagged discrepancy, update the breadcrumb trail accordingly — a flat URL may argue for a two-level Home → Site Preparation trail instead of three).
...
5. **The `/site-preparation/` vs. `/site-preparation-contractor-eugene-oregon/` URL discrepancy flagged in §1** — needs a decision before this page is rebuilt, since it affects the breadcrumb schema, the internal-linking anchors in §9, and whether a redirect is needed.
