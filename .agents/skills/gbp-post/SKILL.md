...
| Post copy rules | `04-gmb/profiles/houston/calendars/master/annual-calendar-2026-2027.md` (standards section only, ~lines 1–40) | Char count target, keyword placement, phone-in-body rule, trust signals, emoji cap |
| Post queue | `04-gmb/profiles/houston/posts/queue.json` | `posted` map = templates used in last ~40 days (exclude these); `history` = recency and cadence |
| Template library | `04-gmb/profiles/houston/posts/templates.json` | 41 existing templates grouped by season; use as starting point or write a new one |
...
Write the post body to the frontmatter spec above. Follow these copy rules (from `04-gmb/profiles/houston/calendars/master/annual-calendar-2026-2027.md`, non-negotiable for this client):
...
**Pre-write checklist — run these greps against your draft title AND body before saving. If any fail, rewrite.**

- `grep -E '\(?713\)?.?598.?2737|tel:'` → zero matches (no phone anywhere in body)
- `grep -cE '[—–;]'` → em dash/en dash/semicolon count must be zero in **both title and body** (numeric ranges `1-3 months` use a hyphen, not en dash)
- `grep -iwE 'ensure|seamless|unlock|leverage|robust|delve|elevate|navigate|top-notch|one-stop|game-changer|rest assured|look no further|in today'` → zero matches (AI slop detector — run against body)
- Post title → max 58 chars; no em dashes, no semicolons (same punctuation rules as body)
...
> **Single-post mode only.** Do NOT use this format in batch mode — batch posts use the leaner format below.
...
**2. Template entry** (if Phase 2 chose "new template") → append to `04-gmb/profiles/houston/posts/templates.json` under the right season key. Use the 300–600 char `summary` field (the pipeline's `copy_builder.py` will expand it to 1200–1450 at publish time). Full template schema in `references/post-formulas.md`.
...
Individual post files in batch mode **must NOT** include a `## Research Notes` section. They include only a `research_refs:` field in frontmatter pointing to the shared RESEARCH_SWEEP.md + the specific data rows that drove this post (e.g., `research_refs: "RESEARCH_SWEEP.md § GSC gap target #2 + Competitor gap bullet"`).

**Correct per-post batch file structure:**
```markdown
---
date: 2026-04-21
post_type: What's New
template_id: pre_summer_refrigerant
primary_keyword: "ac repair cypress tx"
research_refs: "RESEARCH_SWEEP.md § GSC gap target #1, Competitor gap bullet"
[... other frontmatter ...]
---
...
...
```
No Research Notes section. No repeated data from RESEARCH_SWEEP.md. This is the efficiency win of batch mode — research once, reference many times.
...
3. **Commit to one slug per post before writing.** Decide the filename in Phase 3 (the brief) and do not change it. Pattern: `YYYY-MM-DD_post-N-<keyword-fragment>-<geo>.md`. Write the file once. If you need to revise content, use Edit on the same file — never create a second file with a variant slug. A batch directory must contain exactly N post files + RESEARCH_SWEEP.md at the end. If you find extra files from aborted drafts, delete them before finalising.
4. **Rotate across the batch** — this is what makes batch mode valuable vs. 3 single-post calls:
...
5. **Append new templates to `post_templates.json` in a single write** at the end, not per-post.
